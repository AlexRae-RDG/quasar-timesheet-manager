...
export function SettingsScreen() {
  ...
  if (!settings) {
    return <div className="app-shell">Loading settings…</div>;
  }

  const update = (patch: Partial<AppSettings>) =>
    setSettings((prev) => (prev ? { ...prev, ...patch } : prev));

  const setThemeId = (id: string) => update({ themeMode: id });
  const setCustomSeeds = (seeds: PaletteSeeds) => update({ customTheme: seeds });
      ...
      <div className="app-shell">
        ...
        <section className="card">
          ...
          <label className="field field-checkbox">
            <input
              ...
              onChange={(e) => update({ showTimerBar: e.target.checked })}
            />
          ...
          <div className="segmented">
            {HEADER_STYLES.map((opt) => (
              <button
                ...
                onClick={() => update({ headerStyle: opt.id })}
              >
                {opt.label}
              </button>
            ))}
        ...
        <section className="card">
          ...
          <p className="muted">
            ...
            token is stored in this machine's OS keychain -- never in the database, and never
          ...
          <label className="field">
            ...
            <input
              ...
              placeholder={settings.hasJiraToken ? "•••••••• (saved -- enter a new token to replace)" : "Paste your Jira API token"}
            />
          </label>
          <div className="row">
            <button
